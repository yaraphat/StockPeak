    ...
    <div className="min-h-screen bg-[var(--background)] flex items-center justify-center px-6">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="font-bengali text-sm text-[var(--color-muted)] mt-2">আপনার অ্যাকাউন্টে লগইন করুন</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                <label className="text-sm font-medium font-bengali">পাসওয়ার্ড</label>
                <Link href="/forgot-password" className="text-xs text-[var(--color-primary)]">ভুলে গেছেন?</Link>
              </div>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter password" autoComplete="current-password" className="w-full px-3 py-2.5 border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)] text-sm focus:outline-none focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary)]/10 transition" required disabled={loading} />
            </div>
        ...
        <p className="text-center text-sm text-[var(--color-muted)] mt-4">
          অ্যাকাউন্ট নেই?{" "}
          <Link href="/signup" className="text-[var(--color-primary)] font-medium">সাইন আপ করুন</Link>
        </p>
      </div>
    </div>
  );
